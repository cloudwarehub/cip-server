...
RUN apt-get update
RUN apt-get install -y xserver-xorg xserver-xorg-video-dummy wget git autoconf automake libtool
...
RUN git clone https://github.com/libuv/libuv.git && cd libuv && sh ./autogen.sh && make && make install
